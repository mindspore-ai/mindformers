...
用户可以从Hugging Face、ModelScope等开源社区下载所需的模型文件，包括模型权重、Tokenizer、配置等（从头预训练不需加载权重）。 链接如下：
...
parallel_config:
  ...
  model_parallel: 1
context:
  max_device_memory: "59GB"  # Atlas 800T A2/A3 机器建议小于60GB，Atlas 800I A2 机器建议小于30GB
```
...
- max_device_memory：最大设备内存，用于限制模型在单卡上的显存占用。Atlas 800T A2 和 Atlas 900 A3 SuperPoD 等机器建议小于60GB，Atlas 800I A2 机器建议小于30GB。当遇到 HCCL OOM 报错时可适当调低该值，但调低可能会影响模型的显存使用。
...
> 注意：max_device_memory 在 Atlas 800T A2 和 Atlas 900 A3 SuperPoD 等机器上建议小于60GB，在 Atlas 800I A2 上建议小于30GB。
