...
def moe_eplb_print_expert_load():
    """test mcore deepseekv3 train in print_expert_load=True."""
    ms.set_seed(0)
    config = MindFormerConfig(f'{CUR_DIR}/deepseekv3_train.yaml')
    config.print_separate_loss = True
    config.train_precision_sync = True
...
def moe_eplb_expert_relocation():
    """test mcore deepseekv3 train in print_expert_load=True and enable_expert_relocation=True"""
    ms.set_seed(0)
    config = MindFormerConfig(f'{CUR_DIR}/deepseekv3_train.yaml')
    config.print_separate_loss = True
    config.train_precision_sync = True
