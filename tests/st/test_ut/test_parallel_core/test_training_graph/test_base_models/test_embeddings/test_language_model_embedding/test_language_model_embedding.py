...
# ============================================================================
"""Test LanguageModelEmbedding with various configurations"""
...
SINGLE_CARD_TEST_CASES = [
    (
        {"position_embedding_type": "none", "num_tokentypes": 0},
        {"output": "output_only"},
    ),
    (
        {"position_embedding_type": "rope", "num_tokentypes": 0},
        {"output": "output_only"},
    ),
]
...
def build_msrun_command_list(
        ...
        position_embedding_type, num_tokentypes,
    ...
    cmd_list += [
        ...
        f"--num_tokentypes={num_tokentypes}",
        f"--output_path={output_path_param}",
        f"--tensor_parallel={tensor_parallel}"
    ]
    logger.info(f"Equivalent shell command for LanguageModelEmbedding (approximate): {' '.join(cmd_list)}")
    return cmd_list


class TestLanguageModelEmbedding:
    """Test class for LanguageModelEmbedding with different configurations"""
    ...
    def setup_method(self):
        ...
        self.run_script_path = self.sh_path / "run_language_model_embedding.py"
    ...
    def run_test(
        ...
        cmd_list = build_msrun_command_list(
            ...
            num_tokentypes=model_args["num_tokentypes"],
    ...
    def test_language_no_parallel_case(self, model_args, data_keys, tmp_path):
