...
class TestMuonConfig:
    ...
    def test_ns_normalization_uses_fp32_and_returns_bf16(
        ...
        def _fake_norm(tensor, p=None, dim=None, keepdim=False, dtype=None):  # pylint: disable=invalid-name
            events.append(("norm", tensor.dtype, p, dim, keepdim, dtype))
            return _FakeTensor(dtype or tensor.dtype)
        ...
        monkeypatch.setattr(muon_mod.mint, "norm", _fake_norm)
        ...
        assert events == [
            ("norm", input_dtype, "fro", (-2, -1), True, mstype.float32),
