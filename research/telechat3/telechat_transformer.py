...
class TelechatAttention(nn.Cell):
    ...
    def __init__(self, dim: int = 512,
        ...
        if self.hidden_size % self.n_head != 0:
            raise ValueError(
                f"For 'MultiHeadAttention', the class variable 'n_kv_head' must be a multiple of "
        ...
        if self.n_kv_head % parallel_config.model_parallel != 0:
            raise ValueError(
                f"MultiHeadAttention error: n_kv_head ({self.n_kv_head}) must be a multiple of "
                f"parallel_config.model_parallel ({parallel_config.model_parallel})"
            )
