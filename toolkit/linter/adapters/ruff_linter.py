...
def _run_command(
    ...
    *,
    ...
    """ Execute external system commands with enhanced observability and safety."""
...
def run_command(
    ...
    *,
    ...
    """ Retry calling _run_command multiple times"""
    ...
    while True:
        try:
            ...
            logger = logging.getLogger(__name__)
            logger.warning(
                "(%s/%s) Retrying because command timed out: %s",
                retries - remaining_retries,
                retries,
                str(err).split(': ', maxsplit=1)[0],
            )
            time.sleep(1)
...
def get_issue_severity(code: str) -> LintSeverity:
    ...
    """
    advice_code = {"B9", "C4", "E2", "E5", "T400", "T49", "PLC", "PLR"}
    if any(code.startswith(x) for x in advice_code):
...
def check_files(
    ...
    *,
    ...
    """run ruff_linter to check files"""
    try:
        proc = run_command(
            [
                sys.executable,
...
def check_file_for_fixes(
    ...
    *,
    ...
    """run ruff_linter to check files and fix errors"""
    try:
...
def main() -> None:
    ...
    parser.add_argument(
        "--config",
        default=None,
        help="Path to the `toolkit/linter/adapters/pyproject.toml` or `ruff.toml` file to use for configuration",
    )
    parser.add_argument(
        "--explain",
        action="store_true",
    ...
    if args.severity:
        for severity in args.severity:
            ...
            if len(parts) != 2:
                raise ValueError(f"Invalid severity format `{severity}`")
