...
# import triton.language.extra.cann.tl as tl
try:
    from triton.language import extract_slice, insert_slice
except ImportError:
...
def hc_split_sinkhorn_backward_kernel_part1(
    ...
    comb_slice = tl.load(mixes_ptr + pid_feat_off + (2 * hc_mult + arange_val)[None, :], mask=pid_mask[:, None],
                         other=0.0)
...
@ms.ops._ms_pyfunc(infer_func=infer_func_backward)
def hc_split_sinkhorn_triton_backward(
    ...
    mixes = mixes.to(dtype=ms.float32)
    hc_scale = hc_scale.to(dtype=ms.float32)
    hc_base = hc_base.to(dtype=ms.float32)
    grad_pre = grad_pre.to(dtype=ms.float32)
    grad_post = grad_post.to(dtype=ms.float32)
    grad_comb = grad_comb.to(dtype=ms.float32)

    # Initialize gradient tensors with zeros
    grad_mixes = ms.mint.zeros_like(mixes)
    grad_hc_scale = ms.mint.zeros_like(hc_scale)
    grad_hc_base = ms.mint.zeros_like(hc_base)
    comb_tmp = ms.mint.empty((batch_seq_size, hc_mult, hc_mult), dtype=mixes.dtype)
    ...
    grad_mixes_pad = ms.mint.zeros(
        (batch_seq_size, hc_mult, BLOCK_ALIGN),
        dtype=grad_mixes.dtype,
    )
    grad_hc_base_pad = ms.mint.zeros(
        (hc_mult, BLOCK_ALIGN), dtype=grad_hc_base.dtype
    )
...
def infer_func_forward(
        ...
        eps: float):
    b, s, _ = mixes.shape
    pre = ms.mint.empty((b, s, hc_mult), dtype=mixes.dtype)
    post = ms.mint.empty((b, s, hc_mult), dtype=mixes.dtype)
    comb = ms.mint.empty((b, s, hc_mult, hc_mult), dtype=mixes.dtype)
    return pre, post, comb


@ms.ops._ms_pyfunc(infer_func=infer_func_forward)
def hc_split_sinkhorn_triton(
    ...
    pre_flat = ms.mint.empty((batch_seq_size, hc_mult), dtype=mixes.dtype)
    post_flat = ms.mint.empty((batch_seq_size, hc_mult), dtype=mixes.dtype)
    comb_tmp = ms.mint.empty((batch_seq_size, hc_mult, hc_mult), dtype=mixes.dtype)
    ...
    comb_tmp_padded = F.pad(comb_tmp, pad=(0, BLOCK_ALIGN - hc_mult), mode="constant", value=float('-inf'))
    comb_flat_padded = ms.mint.empty((batch_seq_size, hc_mult * BLOCK_ALIGN), dtype=mixes.dtype)
...
class HcSplitSinkhorn(ms.nn.Cell):
    def __init__(self):
        super().__init__()
    ...
    def construct(self,
                  ...
                  sinkhorn_iters: int = 20,
                  eps: float = 1e-6):
        return hc_split_sinkhorn_triton(mixes, hc_scale, hc_base, hc_mult, sinkhorn_iters, eps)

    def bprop(self, mixes, hc_scale, hc_base, hc_mult, sinkhorn_iters, eps, out, dout):
        grad = hc_split_sinkhorn_triton_backward(*dout, mixes, hc_scale, hc_base, hc_mult, sinkhorn_iters, eps)
        return grad[0], grad[1], grad[2], None, None, None
