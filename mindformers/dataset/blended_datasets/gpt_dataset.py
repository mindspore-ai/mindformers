...
class GPTDataset(MegatronDataset):
    ...
    def __init__(
        ...
        try:
            ...
            self._pad_token_id = self.config.pad

        try:
            self._eod_token_id = self.config.tokenizer.eod
        except:
            self._eod_token_id = self.config.eod
    ...
    def _build_document_sample_shuffle_indices(
        ...
        if not path_to_cache or (
            ...
            logger.warning(f"Build and save the {type(self).__name__} {self.index_split.name} indices")
            ...
            # only rank 0 save indices file.
            rank, _ = get_rank_info()
            if path_to_cache and rank == 0:
