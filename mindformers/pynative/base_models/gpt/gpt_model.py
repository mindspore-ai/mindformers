...
class GPTModel(nn.Cell):
    ...
    def take_qk_clip_count(self):
        """Return and clear the device-side QK-clip head count for the monitor callback."""
    ...
    def apply_qk_clip_scaling(self, logit_threshold, fp32_param_map):
        ...
        synced = self._all_reduce_max_logits(
            mint.stack([sa.core_attention.max_logits_val for sa in layers], dim=0))
        # The monitor reads this instead of rebuilding every layer's stat on the host.
        self._qk_clip_count_cache[0] = mint.sum(
            mint.greater(synced, logit_threshold).astype(dtype.int32)).reshape((1,))
        scales = logit_threshold / mint.clamp(synced, min=logit_threshold)
